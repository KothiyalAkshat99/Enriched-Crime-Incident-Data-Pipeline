...
def get_weather(db: sqlite3.Connection) -> None:
    """Fetch weather data for each location in the database."""
    url = "https://archive-api.open-meteo.com/v1/archive"
    cur = db.cursor()
    locations = cur.execute("SELECT DISTINCT datetime, location, latitude, longitude FROM incidents JOIN location ON incidents.location = location.loc").fetchall()
    ...
    for datetime_str, location, latitude, longitude in locations:
        ...
        try:
            ...
            if hour < len(hourly_weather_code):
                ...
                cur.execute("UPDATE incidents SET weather = ? WHERE datetime = ? AND location = ?", (weather_code, datetime_str, location))
            else:
